...
func AddAlbum(w http.ResponseWriter, r *http.Request, enc Encoder, db DB) (int, string) {
	...
	case ErrAlreadyExists:
		return http.StatusConflict, Must(enc(NewError(ErrCodeAlreadyExists, fmt.Sprintf("the album '%s' from '%s' already exists", al.Title, al.Band))))
	case nil:
...
func GetAlbum(enc Encoder, db DB, parms martini.Params) (int, string) {
	...
	if err != nil || al == nil {
		...
		return http.StatusNotFound, Must(enc(NewError(ErrCodeNotExist, fmt.Sprintf("the album with id %s doese not exist", parms["id"]))))
	}
